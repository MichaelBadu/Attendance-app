...
private void listSwipeStatistics() {
        ...
        for(int i  = 0; i < this.repository.getItems().size(); i++){
            if(this.repository.getItems().get(i).getRoom().equals(temp)){
                timeStamp =  this.repository.getItems().get(i).getSwipeDate();
                counter++;
            }
        }
